...
* Enviroment
  - GNU Emacs 28.0.50 (build 1, x86_64-pc-linux-gnu, GTK+ Version 3.24.26, cairo version 1.17.4) of 2021-02-27
  - build branch: feature/native-comp 42fc75
  - system-configuration-options: ~CFLAGS='-O3' --with-x-toolkit=gtk3 --with-native-compilation --with-imagemagick~
...
** Or Install gccemacs, Bringing GNU Emacs to Native Code (Recommended)
   ...
   #+begin_src shell
     ...
     ./autogen.sh
     ./configure CFLAGS='-O3' --with-x-toolkit=gtk3 --with-native-compilation --with-imagemagick
     make -j8
     sudo make install
   #+end_src
...
** pgtk-nativecomp (eaf broken)
   ...
   #+begin_src shell
     ...
     ./configure CFLAGS='-O3' --with-pgtk --with-native-compilation --with-imagemagick
     make -j8
     sudo make install
   #+end_src
