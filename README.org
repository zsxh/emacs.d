...
* Enviroment
  - GNU Emacs 28.0.50 (build 1, x86_64-pc-linux-gnu, GTK+ Version 3.24.22, cairo version 1.17.3) of 2020-08-29
  - build branch: feature/native-comp commit aa526c9
  - system-configuration-options: ~--with-xwidgets --with-x-toolkit=gtk3 --with-nativecomp --with-imagemagick~
...
* TODOs
  ...
  - [ ] dwim evil + vterm + tmux + project (+ leader key for vterm?)
...
** Build Emacs 28
   #+begin_src shell
     ...
     cd emacs
     ./autogen.sh
     ./configure --with-xwidgets --with-x-toolkit=gtk3 --with-imagemagick
     make -j8
     sudo make install
   #+end_src
...
** Or Install gccemacs, Bringing GNU Emacs to Native Code (Recommended)
   ...
   #+begin_src shell
     ...
     ./autogen.sh
     ./configure --with-xwidgets --with-x-toolkit=gtk3 --with-nativecomp --with-imagemagick
     make -j8
     sudo make install
   #+end_src
