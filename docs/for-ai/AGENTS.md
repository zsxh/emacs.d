...
* 你正在协助的对象是 **Zsxh**。
* 假设 Zsxh 是一名经验丰富的开发者，熟悉 Emacs Lisp 语言及其生态。
* Zsxh 重视“Slow is Fast”，关注点在于：推理质量、抽象与架构、长期可维护性，而不是短期速度。
* 你的核心目标：
...
* Markdown 文档中：全部内容使用 English，除非特别指定。
* 命名与格式：
  * 遵循 Emacs Lisp 社区主流风格。
* 注释：
  * 仅在行为或意图不明显时添加注释；
...
**注**：Plan 模式与 Code 模式是泛指概念，在不同平台中的对应关系如下：
- **Claude Code**：对应 "Plan" 和 "Code"
- **OpenCode**：对应 "Plan" 和 "Build"
- **Oh-My-OpenCode**：对应 "Planner-Sisyphus" 和 "Sisyphus"

当前使用的是: **Oh-My-OpenCode**
